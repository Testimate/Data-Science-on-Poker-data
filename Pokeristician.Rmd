...
date: "Nov 16, 2018, Latest Data Update: Mar 11st, 2019"
...
_Dec 5th: Idea: separate 1st to 14th session (Sat) with the 15th  (Thu) and the following session pre-"The Course" era and post-"The Course" era: Game change mindset after discover and read The Book by Ed Miller (at Capen Library 3rd floor, midnight)_

_Dec 5th: Implementation: manuever a separate dataset 'Enlightened' - a subset corresponds to the aforementioned definition _

_Idea: Hypothesis: The Book helps significantly._
...
highchart() %>%
  ...
  hc_xAxis(categories = Testimate %>% select(Day) %>% .[[1]], # or day
           plotBands = list(
              ...
              list(from = 29.5, to = 30.5, color = "#BDFCC9"),
              list(from = 30.5, to = 31.5, color = "#BDFCC9")
              )) %>%
...
highchart() %>%
  ...
  hc_xAxis(categories = EnlightenedEra %>% select(Day) %>% .[[1]], # or day
           plotBands = list(
              ...
              list(from = 15.5, to = 16.5, color = "#BDFCC9"),
              list(from = 16.5, to = 17.5, color = "#BDFCC9")
              )) %>%
  hc_yAxis(plotLines = list(
              list(value = 0, width = 5, color = 'black')
              ))
