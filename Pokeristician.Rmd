...
date: "Nov 16, 2018, Latest Update: Feb 15, 2019"
...
highchart() %>%
  ...
  hc_xAxis(categories = Testimate %>% select(Day) %>% .[[1]], # or day
           plotBands = list(
              ...
              list(from = 22.5, to = 23.5, color = "#CAFF70")
              )) %>%
...
series = list(
  list(
    name = 'Total Profit for each session in Enlightened Era (promo bonus included), stratified by stake [yellow: 1/2, green: 1/3, lime: tough lineup]',
    color = '#1E90FF', # <http://cloford.com/resources/colours/500col.htm>
...
highchart() %>%
  ...
  hc_xAxis(categories = EnlightenedEra %>% select(Day) %>% .[[1]], # or day
           plotBands = list(
              ...
              list(from = 8.5, to = 9.5, color = "#CAFF70")
              )) %>%
  hc_yAxis(plotLines = list(
              list(value = 0, width = 3, color = 'black')
              ))
