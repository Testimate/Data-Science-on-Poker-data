...
date: "Nov 16, 2018, Latest data set update: Mar 14th, 2019"
...
_Dec 5th: Idea: separate 1st to 14th session (Sat) with the 15th  (Thu) and the following session pre-"enlightment" era and post-"enlightment": (That means the inspiration and mindset I got from after reading the Book 'The Course' by Ed Miller)_

_Dec 5th: Implementation: manuever a separate dataset 'Enlightened'_

_Hypothesis: The Book helps significantly._


_Mar 16th: Idea: Build a ShinyApp that_
...
highchart() %>%
  ...
  hc_xAxis(categories = Testimate %>% select(Day) %>% .[[1]], # or day
           plotBands = list(
              ...
              list(from = 29.5, to = 32.5, color = "#BDFCC9")
              )) %>%
...
highchart() %>%
  ...
  hc_xAxis(categories = EnlightenedEra %>% select(Day) %>% .[[1]], # or day
           plotBands = list(
              ...
              list(from = 15.5, to = 18.5, color = "#BDFCC9")
              )) %>%
  hc_yAxis(plotLines = list(
              list(value = 0, width = 5, color = 'black')
              ))
